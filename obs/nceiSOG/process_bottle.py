"""
Code to process the bottle profile carbon data collected in the Strait of Georgia, 275 casts

NOTE: the code cropped out JDF region to focus on SOG only,since nceiSalish dataset already covered JDF region extensively
There are multiple files for a given year, or multiple datasets for different year. 
So I combined all the files and then divide to yearly for easier process
...
for ncei in ncei_list.keys():
    ...
    if ncei == '0173514':
        ...
        df1 = df1[~((df1.lon > -125) & (df1.lon < -123.1) & (df1.lat > 48) & (df1.lat < 48.6))]
        # the cid
        ...
        df3 = df3[(df3.lon > -124.98) & (df3.lon < -122.95) & (df3.lat > 48.5) & (df3.lat < 51.5)]
        ...
        df4 = df4[~((df4.lon > -126) & (df4.lon < -124) & (df4.lat > 48) & (df4.lat < 49.1))]
